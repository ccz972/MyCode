...
void solve() {
  ...
  auto chk = [&] (int x) -> bool {
    ...
    vector<vector<pair<int, int> > > ne(n, vector<pair<int, int> >(2, {-1, -1}));
    ...
    for(int i = 1; i < n; i ++ ) if(ne[i - 1][1] > ne[i][0]) {
      ne[i][0] = ne[i - 1][1];
      if(ne[i][0] > ne[i][1]) {
        swap(ne[i][0], ne[i][1]);
      }
      ...
      if(ne[i][0].second == ne[i][1].second) {
        // debug(x, ne[i][0], ne[i][1]);
        // cout << x << " " << ne[i][0].second << " " << ne[i][1].second << "\n";
        exit(0);
      }
    }
