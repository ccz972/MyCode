...
bool in(TPoint a, TPoint u, TPoint v) {
  TLine l = get_line(u, v);
  if(sign(a.x * l.a + a.y * l.b + l.c) == 0) return true;
  return false;
}
...
int main()
{
    ...
    for(int id=1;id<=T;id++)
    {
        ...
        // if(id < 23) continue;
        // if(id == 24) print(B[0]), print(B[1]), print(W[0]), print(W[1]); 
        ...
        if(seg_inter(B[0],B[1],W[0],W[1]))
        {
            ...
            // print(res);
            // if(on_segment(res,B[0],B[1])) puts("FUCHK");
            // else puts("FFFF");
            if(on_segment(res, B[0], B[1]) && res != B[0] && res != B[1]) {
              if(res == W[0] || res == W[1]) {
                cout << "0\n";
              } else {
                cout << "inf\n";
