...
void solve() {
  ...
  int x = 0, y = 0;
  for (int i = 0; i < (1 << (2 * n)); i ++ ) if (int(__builtin_popcount(i)) == n) {
    vector<int> b, c;
    Rep(j, 0, 2 * n) if (i >> j & 1) {
      b.eb(a[j]);
    } else {
      c.eb(a[j]);
    }
...
mint dfs(int n, int m) {
  if (n == 1) return mint(1);
  mint now = C(n - 1, n / 2 - 1);
  mint p = (m ? C(2 * m, m) / m : mint(1));
  return p * dfs(n - 2, m + 1) + now;
}
